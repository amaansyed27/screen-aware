...
The MCP server exposes `screen_aware_watch_live_issue` for these prompts. It keeps the tool call open while you demonstrate the issue, then returns visual/audio/annotation/note evidence to the agent. In diagnostic mode the agent should report and ask before changing files. In live edit mode the agent should immediately inspect and patch after the watch window.
