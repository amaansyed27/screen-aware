...
The MCP server exposes `screen_aware_watch_live_issue` for these prompts. By default it keeps the tool call open until the companion emits `capture.stopped`, then returns visual/audio/annotation/note evidence to the agent. In diagnostic mode the agent should report captured observations and ask before changing files. In live edit mode the agent should immediately inspect and patch after the stopped capture returns evidence.
