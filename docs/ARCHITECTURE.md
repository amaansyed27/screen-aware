...
- Live model keys belong only in `.env` or user environment variables. They are never sent to the companion UI. The default provider is OpenRouter with `deepseek/deepseek-v4-flash:free` as primary; direct Gemini API is available with `SCREEN_AWARE_LIVE_PROVIDER=gemini` and `gemini-3.1-flash-lite`.
